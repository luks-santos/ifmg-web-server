...
void serverResponse::handleRequest(int thread_id, int clientSockfd, sockaddr_in clientAddr, string dir){
...
vector<string> serverResponse::split(string str, char del){
...
int serverResponse::handleGet(int thread_id, int clientSockfd, sockaddr_in clientAddr, string root,string extension, string dir){
    string n, data;
    int len;
    if(Debug){
        cout<< "Root " << root << endl;
...
bool serverResponse::arqStream(string root, int &len, string &n) { //Abertura do arquivo
    fstream file;
...
void serverResponse::readFile(fstream &file, string &n, int len) {
...
string serverResponse::getStatus(int resp, int len, string extension) {
    string text;
    mimetype mim;
    char buff[256];
    time_t rawtime;
